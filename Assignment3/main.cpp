...
// #include "stdafx.h" //Uncomment when working with Visualstudio
#include <iostream>
#include <cmath>
#include <cstdlib>
#include <cassert>
...
int main()
{
        ...
        // Initialize values f will take.
        std::vector<double> runForce{ 0.0, 1.0e-12, 2.0e-12,
                                      5.0e-12, 1.0e-11, 2.0e-11 };

        const int nTotal        = 13;                                   // total number of filaments
        const double tEnd       = 200.0;                                // minutes for every simulation
        const double maxRuns    = 3.0;

        const double d          = 8.0e-9;                               // size of a subunit in meters
        const int maxSimNum     = static_cast<int>(runForce.size());    // number of simulations in the vector runForce
        const double h          = d / nTotal;
        const double temp       = 298.0;                                // temperature in kelvin
        ...
        // for loop for maxRuns for forces maxSimNum simulations will be)
        int simNum = 0;
        for (double run = 0.0, force = 0.0; simNum < maxSimNum;)
        {
                force = runForce[simNum];
                std::cout << run << '\n';
        ...
        )
{
        ...
        for (int rate = 0; rate < static_cast<int>(lFils.size()); ++rate)
        {
                int lNeighbour = 0, rNeighbour = 0;
                if (rate == 0)
                        lNeighbour = static_cast<int>(lFils.size()) - 1;
                else
                        lNeighbour = rate - 1;
                if (rate == static_cast<int>(lFils.size()) - 1)
                        rNeighbour = 0;
                else
                        rNeighbour = rate + 1;
